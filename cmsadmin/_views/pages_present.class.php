...
class pagesPresent {
    ...
    public function show_pages_overview($overview, $area, $action, $msg) {
        ...
        while ($row = $overview->fetchRow(MDB2_FETCHMODE_ASSOC)) {
            ...
            if ($fach != $fach_save) {
                ...
                $utf8_fach = utf8_encode($fach);
                if ($_COOKIE[$utf8_fach] == '') {
                    $_COOKIE[$utf8_fach] = 'close';
                }
            } else {
                $tpl->setVariable('class_gruppe', 'hide');
            }

            if ($_COOKIE[$utf8_fach] == 'open') {
                $display = '';
            ...
            if ($fach != $fach_save) {
                ...
                $fach_save = $fach;
                // Gruppenbruch - Titel aus Navigation ermitteln
                $navid = $row['nav_id'];
                $navigation = $navi->read_navigation_by_id($navid); 
                $gruppentitel = $navigation['bezeichnung'];
            ...
            $tpl->setVariable('fach', $gruppentitel);
        ...
        if ($i == 0) {
            $msg[] = 'error';
            $msg[] = $GLOBALS['MESSAGES']['MSG_NOTHING_FOUND'];
        }
    ...
    private function date_mysql2german($datum) {
        if (!$general) {
            $general = new generalPresent($this->mTpl);
        }
