...
switch ($action) {
    ...
    case 'typeSave':
        $wert = isset($_GET['wert']) ? $_GET['wert'] : '';
        if ($wert == $GLOBALS['NAVI']['HORIZONTAL']) {
            $feldwert = $GLOBALS['NAVI']['VERTIKAL'];
        } elseif ($wert == $GLOBALS['NAVI']['VERTIKAL']) {
            $feldwert = $GLOBALS['NAVI']['SERVICE'];
        } elseif ($wert == $GLOBALS['NAVI']['SERVICE']) {
            $feldwert = $GLOBALS['NAVI']['HORIZONTAL'];
        }
...
elseif ($action == 'del' && $nav_id != 0) {
    ...
    if ($row['kap'] != 0 && $row['ukap'] == 0) {
        ...
        if ($count > 0) {
            $message[] = 'error';
            $message[] = sprintf($GLOBALS['MESSAGES']['MSG_NAVI_NICHT_LOESCHBAR'], $count);
        } else {
