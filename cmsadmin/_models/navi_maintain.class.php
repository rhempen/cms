...
class naviMaintain {
    ...
    public function kap_hochziehen($kap) {
        ...
        if ($db->query('UPDATE ' . $this->mPrefix . 'navigation SET kap=222 WHERE kap=' . $kap) &&
            ...
            $this->mMeldung[] = sprintf($GLOBALS['MESSAGES']['MSG_KAP_HOCHZIEHEN'], $GLOBALS['row']['bezeichnung']);
    ...
    public function ukap_hochziehen($nav_id) {
        ...
        if ($db->query('UPDATE ' . $this->mPrefix . 'navigation SET ukap=' . $ukap . ' WHERE kap=' . $kap . ' AND ukap=' . ($ukap - 10)) &&
            ...
            $this->mMeldung[] = 'success';
            $this->mMeldung[] = sprintf($GLOBALS['MESSAGES']['MSG_UKAP_HOCHZIEHEN'], $GLOBALS['row']['bezeichnung']);
            return $this->mMeldung;
        }
    }
    ...
    public function neuer_ukap($nav_id) {
        ...
        $media_dir = $domain.'_'.$ukap_nummer;
        ...
        if (!is_dir($dir_to_create) && mkdir($dir_to_create)) {
            $this->mMeldung[] = 'success';
            $this->mMeldung[] = sprintf($GLOBALS['MESSAGES']['MSG_DIR_ANGELEGT'], $bildpfad);
        } elseif (!is_dir($dir_to_create)) {
            $this->mMeldung[] = 'error';
            $this->mMeldung[] = sprintf($GLOBALS['MESSAGES']['MSG_DIR_NICHT_ANGELEGT'], $bildpfad);
        }
        ...
        if ($db->query($insert_navi) && $db->query($insert_redirect) && $db->query($insert_page)) {
            $this->mMeldung[] = 'success';
            $this->mMeldung[] = sprintf($GLOBALS['MESSAGES']['MSG_NAVI_SEITE_ANGELEGT'], $kap_bez);
        } else {
            $this->mMeldung[] = 'error';
            $this->mMeldung[] = sprintf($GLOBALS['MESSAGES']['MSG_NAVI_SEITE_NICHT_ANGELEGT'], $kap_bez);
        }
        return $this->mMeldung;
    }
    ...
    public function neuer_kap() {
        ...
        if (!is_dir($dir_to_create) && mkdir($dir_to_create)) {
            ...
            $this->mMeldung[] = 'warning';
            $this->mMeldung[] = sprintf($GLOBALS['MESSAGES']['MSG_DIR_EXISTS'], $bildpfad);
        } elseif (!is_dir($dir_to_create)) {
            $this->mMeldung[] = 'error';
            $this->mMeldung[] = sprintf($GLOBALS['MESSAGES']['MSG_DIR_NICHT_ANGELEGT'], $bildpfad);
        }
    ...
    public function delete_navigationspunkt($row, $msg) {
        ...
        if ($kap != 0 && $ukap != 0) {
            ...
            if ($navid != 0) {
                $delete = 'DELETE FROM ' . $this->mPrefix . 'redirect WHERE navid=' . $navid . ' AND subid=' . $nav_id . ' AND pagid=0';
            }
        } else {
            // es wird das ganze Kapitel geloescht, also alle Eintraege zur nav_id loeschen
            if ($navid != 0) {
                $delete = 'DELETE FROM ' . $this->mPrefix . 'redirect WHERE navid=' . $nav_id;
            }
        }
        if ($delete != '') {
            $db->query($delete);
        ...
        if ($db->query($delete) && $db->query($update) && $db->query($delete_page)) {
            $msg[] = 'success';
            $msg[] = sprintf($GLOBALS['MESSAGES']['MSG_NAVI_SEITE_GELOESCHT'], $seite);
        } else {
            $msg[] = 'error';
            $msg[] = sprintf($GLOBALS['MESSAGES']['MSG_NAVI_SEITE_NICHT_GELOESCHT'], $seite);
        }
        return $msg;
    }
    ...
    public function copy_navi_page($navigation, $navi_page) {
        ...
        if ($db->query($update_kap) && $db->query($insert_navi) && $db->query($insert_redirect) && $db->query($insert_seite)) {
            $this->mMeldung[] = 'success';
            $this->mMeldung[] = sprintf($GLOBALS['MESSAGES']['MSG_NAVI_SEITE_ANGELEGT'], $row_navi['kuerzel']);
        } else {
            $this->mMeldung[] = 'error';
            $this->mMeldung[] = sprintf($GLOBALS['MESSAGES']['MSG_NAVI_SEITE_NICHT_ANGELEGT'], $row_navi['kuerzel']);
        }
        return $this->mMeldung;
    }
    ...
    private function prepare_copy_kap($navigation) {
        ...
        $row_navi['domain']     = 'navi_' . $row_navi['nav_id'];
    ...
    public function save_navi_page($seiten_id) {
        ...
        if (PEAR::isError($affected)) {
            $msg[] = 'error';
            $msg[] = $affected->getMessage() . ' ' . $affected->getDebugInfo();
        } else {
            $msg[] = 'success';
            $msg[] = sprintf($GLOBALS['MESSAGES']['MSG_NAVI_SEITE_GESPEICHERT'], $_POST['kurztitel']);
        }
        return $msg;
    }
    ...
    public function navi_sortieren($srtids) {
        ...
        if ($error != 0) {
            echo $GLOBALS['MESSAGES']['MSG_NICHT_NEU_SORTIERT'] . "<br />\n";
        } else {
            echo $GLOBALS['MESSAGES']['MSG_NEU_SORTIERT'];
        }
    }
...
//		$newpath  = ROOTDIR.$newmedia;
        ...
        if (count($seiten) > 0) {
            while ($row = $seiten->fetchRow(MDB2_FETCHMODE_ASSOC)) {
                ...
                $inhalt2 = $row['inhalt2'];
                $bild1 = $row['bild1'];
                if (stristr($inhalt1, '<img')) {
                    $inhalt1 = str_replace('/media/', $newmedia, $row['inhalt1']);
                }
                if (stristr($inhalt2, '<img')) {
                    $inhalt2 = str_replace('/media/', $newmedia, $row['inhalt2']);
                }
                if (stristr($bild1, '/media/')) {
                    $bild1 = str_replace('/media/', $newmedia, $row['bild1']);
                } {
                    ...
                    if ($inhalt1 != $row['inhalt1'] || $inhalt2 != $row['inhalt2'] || $bild1 != $row['bild1']) {
                        ...
                        if (PEAR::isError($affected)) {
                            $msg[] = 'error';
                            $msg[] = sprintf($GLOBALS['MESSAGES']['MSG_NAVI_SEITE_NICHT_GESPEICHERT'], $seite);
                        } else {
                            $msg[] = 'success';
                            $msg[] = sprintf($GLOBALS['MESSAGES']['MSG_NAVI_SEITE_GESPEICHERT'], $seite);
                        }
                    } else {
                        $msg[] = 'neutral';
                        $msg[] = 'OK: ' . $seite;
