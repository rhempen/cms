...
class picturesDbMaintain {
            ...
            case 'insert':
                if ($bild_id == null) {
                    ...
                    if (PEAR::isError($affected)) {
                        ...
                        $msg[] = 'error';
                        $msg[] = sprintf($GLOBALS['MESSAGES']['MSG_BILD_NICHT_IN_DB_GESPEICHERT'], $bild);
                    } else {
                        $msg[] = 'success';
                        $msg[] = sprintf($GLOBALS['MESSAGES']['MSG_BILD_IN_DB_GESPEICHERT'], $bild);
                    }
                } else {
                    $msg[] = 'error';
                    $msg[] = sprintf($GLOBALS['MESSAGES']['MSG_BILD_EXISTIERT_IN_DB'], $bild);
                }
                break;
            default:
                if ($bild_id != null) {
                    ...
                    $msg[] = 'error';
                    $msg[] = sprintf($GLOBALS['MESSAGES']['MSG_BILD_NICHT_IN_DB'], $bild);
                }
        }
        return $msg;
    }
    ...
    public function bild_in_db_loeschen($ref_id, $pfad_bild, $msg) {
        ...
        if ($bild_id != null) {
            ...
            if ($db->exec($delete) && $db->query($update_sortkey)) {
                $msg[] = 'success';
                $msg[] = sprintf($GLOBALS['MESSAGES']['MSG_BILD_IN_DB_GELOESCHT'], $bild);
            } else {
                $msg[] = 'error';
                $msg[] = sprintf($GLOBALS['MESSAGES']['MSG_BILD_NICHT_IN_DB_GELOESCHT'], $bild);
            }
        } else {
            $msg[] = 'error';
            $msg[] = sprintf($GLOBALS['MESSAGES']['MSG_BILD_NICHT_IN_DB'], $bild);
    ...
    public function bilder_loeschen_pro_ref_id($ref_id, $type, $msg = '') {
        if ($ref_id > 0 && ($type == 'N' || $type == 'P')) {
            ...
            if ($rid == $ref_id) {
                ...
                if ($db->exec($delete)) {
                    $msg[] = 'success';
                    $msg[] = sprintf($GLOBALS['MESSAGES']['MSG_BILDER_PRO_REF_GELOESCHT'], $seite);
                } else {
                    $msg[] = 'error';
                    $msg[] = sprintf($GLOBALS['MESSAGES']['MSG_BILDER_PRO_REF_NICHT_GELOESCHT'], $seite);
                }
            }
        }
        return $msg;
    ...
    public function bild_hochziehen($ref_id, $sortkey) {
        ...
        if ($db->query('UPDATE ' . $this->mPrefix . 'galerien SET sortkey=222 WHERE ref_id=' . $ref_id . ' AND sortkey=' . $sortkey) &&
            ...
            $msg[] = 'success';
            $msg[] = $GLOBALS['MESSAGES']['MSG_BILD_HOCHGEZOGEN'];
            return $msg;
        }
    }
...
//		var_dump(mb_detect_encoding($kommentar, "UTF-8"));
    ...
    public function update_bildpfade() {
        global $db, $msg;
        $msg[] = 'message';
        $msg[] = $GLOBALS['LABELS']['BILDERGALERIE'];
        ...
        if (count($bilder) > 0) {
            while ($row = $bilder->fetchRow(MDB2_FETCHMODE_ASSOC)) {
                if (!stristr($row['bildpfad'], $newmedia)) { // Update nur, wenn nötig!
                    ...
                    if (PEAR::isError($affected)) {
                        $msg[] = 'error';
                        $msg[] = sprintf($GLOBALS['MESSAGES']['MSG_BILD_NICHT_IN_DB_GESPEICHERT'], $pfad);
                    } else {
                        $msg[] = 'success';
                        $msg[] = sprintf($GLOBALS['MESSAGES']['MSG_BILD_IN_DB_GESPEICHERT'], $pfad);
                    }
                } else {
                    $msg[] = 'neutral';
                    $msg[] = 'OK: ' . $row['bildpfad'];
    ...
    public function refresh_sortkey($ref_id, $type) {
        ...
        while ($bild = $bilder->fetchRow(MDB2_FETCHMODE_ASSOC)) {
            ...
            $msg[] = 'success';
            $msg[] = 'Sortkey/Type/Id/Ref_Id/Bildpfad: ' . $sortkey . ' ' . $bild['type'] . ' ' . $bild['id'] . ' ' . $bild['ref_id'] . ' ' . $bild['bildpfad'];
        }
        return $msg;
    }
    ...
    private function adjust_galerien_navi() {
        ...
        if (count($navigation) > 0) {
            while ($row = $navigation->fetchRow(MDB2_FETCHMODE_ASSOC)) {
                ...
                $msg[] = 'title';
                $msg[] = 'Navid=' . $row['nav_id'] . ' Path: ' . $row['bildpfad'] . ' Nav/Unav: ' . $row['kap'] . '/' . $row['ukap'];
    ...
    private function adjust_galerien_pages() {
        ...
        while ($row = $subpages->fetchRow(MDB2_FETCHMODE_ASSOC)) {
            // 2. Dateien pro Bildpfad einlesen
            $msg[] = 'title';
            $msg[] = 'Pagid/Navid=' . $row['page_id'] . '/' . $row['nav_id'] . ' Path: ' . $row['kennzeichen'];
