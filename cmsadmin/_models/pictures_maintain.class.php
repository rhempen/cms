...
//                $stat_array = stat($file);
        ...
        if (is_dir($dir)) {
            ...
            if (file_exists($fileold)) {
                if (rename($fileold, $file)) {
                    ...
                    $this->mMeldung[] = $GLOBALS['MESSAGES']['MSG_DATEI_UMBENANNT'];
                } else {
                    $this->mMeldung[] = 'error';
                    $this->mMeldung[] = $GLOBALS['MESSAGES']['MSG_DATEI_NICHT_UMBENANNT'];
                }
            }
        }
    ...
    public function delete_file($dir, $file) {
        ...
        if (is_dir($dir)) {
            ...
            if (file_exists($datei) && is_dir($datei)) {
                ...
                if ($count > 0) {
                    $this->mMeldung[] = 'error';
                    $this->mMeldung[] = sprintf($GLOBALS['MESSAGES']['MSG_DIR_NICHT_LEER'], $file);
                } else {
    ...
    public function deltree($path) {
        ...
        if (is_dir($path)) {
            ...
            if ($count_err_dire != 0) {
                $this->mMeldung[] = 'error';
                $this->mMeldung[] = sprintf($GLOBALS['MESSAGES']['MSG_DIR_NICHT_GELOESCHT'], $path);
            } else {
                $this->mMeldung[] = 'success';
                $this->mMeldung[] = sprintf($GLOBALS['MESSAGES']['MSG_DIR_GELOESCHT'], $path);
            }
        } elseif (is_file($path)) {
            if (!unlink($path)) {
                $this->mMeldung[] = 'error';
                $this->mMeldung[] = sprintf($GLOBALS['MESSAGES']['MSG_DATEI_NICHT_GELOESCHT'], $path);
            }
        }
        return $this->mMeldung;
    }
    ...
    private function datei_loeschen($datei) {
        ...
        if (file_exists($datei)) {
            if (unlink($datei)) {
                $this->mMeldung[] = 'success';
                $this->mMeldung[] = sprintf($GLOBALS['MESSAGES']['MSG_DATEI_GELOESCHT'], $file);
            } else {
                $this->mMeldung[] = 'error';
                $this->mMeldung[] = sprintf($GLOBALS['MESSAGES']['MSG_DATEI_NICHT_GELOESCHT'], $file);
            }
        }
    }
    ...
    private function verzeichnis_loeschen($verzeichnis) {
        ...
        if (rmdir($verzeichnis)) {
            $this->mMeldung[] = 'success';
            $this->mMeldung[] = sprintf($GLOBALS['MESSAGES']['MSG_DIR_GELOESCHT'], $dir);
        } else {
            $this->mMeldung[] = 'error';
            $this->mMeldung[] = sprintf($GLOBALS['MESSAGES']['MSG_DIR_NICHT_GELOESCHT'], $dir);
        }
    }
    ...
    public function scan_directories($srcdir = MEDIA_ROOT, $verzeichnisse = array()) {
        // Erster Eintrag = MEDIA_ROOT = /bilder
        if ($verzeichnisse[0] != $srcdir) {
            $verzeichnisse[0] = MEDIA_ROOT;
        }
        // Beginn Scanning
        ...
        foreach ($verz as $key => $subdir) {
            ...
            if (preg_match('/media/', $dir) && $dir != MEDIA_ROOT) {
                if (!file_exists($dir)) {
                    if (mkdir($dir)) {
                        $this->mMeldung[] = 'success';
                        $this->mMeldung[] = sprintf($GLOBALS['MESSAGES']['MSG_DIR_ANGELEGT'], $dir);
                    }
                }
            }
        }
    ...
    public function upload_files($aktdir = '') {
        ...
        for ($count = 1; $count <= 10; $count++) {
            ...
            if ($dateiname != '' && $dateitype != '') {
                ...
                if ($subrc != 0) {
                    $this->mMeldung[] = 'error';
                    $this->mMeldung[] = sprintf($GLOBALS['MESSAGES']['MSG_FALSCHER_DATEITYP'], $dateiname);
                }
                ...
                if ($subrc == 0 && $dateitype != 'application/pdf') {
                    ...
                    if ($subrc != 0) {
                        ...
                        $this->mMeldung[] = 'error';
                        $this->mMeldung[] = sprintf($GLOBALS['MESSAGES']['MSG_DATEI_ZU_GROSS'], $dateiname, $size_kb);
                    }
                }
                ...
                if ($subrc == 0 && $aktdir != '') {
                    ...
                    if (file_exists($path_file)) {
                        $this->mMeldung[] = 'error';
                        $this->mMeldung[] = sprintf($GLOBALS['MESSAGES']['MSG_DATEI_EXISTIERT_SCHON'], $dateiname);
                    } else {
                        ...
                        if (move_uploaded_file($dateitemp, $path_file) && chmod($path_file, 0755)) {
                            $this->mMeldung[] = 'success';
                            $this->mMeldung[] = sprintf($GLOBALS['MESSAGES']['MSG_DATEI_HOCHGELADEN'], $dateiname);
                            ...
                            if ($file_width > IMAGE_MAX_WIDTH || $file_height > IMAGE_MAX_HEIGHT) {
                                ...
                                $this->mMeldung[] = 'success';
                                $this->mMeldung[] = sprintf($GLOBALS['MESSAGES']['MSG_DATEI_RESIZED'], $dateiname, IMAGE_MAX_WIDTH, IMAGE_MAX_HEIGHT);
                            }
                        } else {
                            $this->mMeldung[] = 'error';
                            $this->mMeldung[] = sprintf($GLOBALS['MESSAGES']['MSG_DATEI_NICHT_HOCHGELADEN'], $dateiname);
                        }
    ...
    private function check_subdirectories($pfad) {
        ...
        if (!file_exists($pfad)) {
            if (mkdir($pfad)) {
                $this->mMeldung[] = 'success';
                $this->mMeldung[] = sprintf($GLOBALS['MESSAGES']['MSG_DIR_ANGELEGT'], $pfad);
                ;
            } else {
                $this->mMeldung[] = 'error';
                $this->mMeldung[] = sprintf($GLOBALS['MESSAGES']['MSG_DIR_NICHT_ANGELEGT'], $pfad);
            }
        }
        return $msg;
    }
    ...
    private function create_thumbnail($image_file, $thumb_path, $breite, $hoehe) {
        ...
        if ($gd_version) {
            ...
            if ($subrc && chmod($thumb_image, 0755)) {
                $this->mMeldung[] = 'success';
                $this->mMeldung[] = sprintf($GLOBALS['MESSAGES']['MSG_THUMBNAIL_GESPEICHERT'], $image, $thumbnail);
            } else {
                $this->mMeldung[] = 'error';
                $this->mMeldung[] = sprintf($GLOBALS['MESSAGES']['MSG_THUMBNAIL_NICHT_GESPEICHERT'], $image);
            }
        }
    }
    ...
    public function resize_picture($image_file_path, $new_image_file_path, $max_width = 100, $max_height = 100) {
        ...
        if ($gd_version == "gd2") {
            ...
            echo "GD Image Library is not installed.";
