...
class pagesMaintain {
    ...
    public function read_pages_overview($area, $referrer = '', $fach = '', $start = 0) {
        global $db;
        $where = ' domain="' . $area . '"';
        $felder = 'page_id, nav_id, kennzeichen, fach, name, datum_von, datum_bis, aktiv';
//		$order  = 'fach, datum_von DESC, name';
    ...
    public function move_up_page($page_id, $sort_id, $fach, $name) {
        ...
        if ($sort_id > 1) {
            if ($db->query('UPDATE ' . $this->mPrefix . 'pages SET sort_id=222 WHERE page_id=' . $page_id) &&
                ...
                $this->mMsg[] = 'success';
                $this->mMsg[] = sprintf($GLOBALS['MESSAGES']['MSG_KAP_HOCHZIEHEN'], $name);
                return $this->mMsg;
            }
        }
    }
    ...
    public function delete_page($page_id, $msg) {
        ...
        if ($db->query($delete)) {
            $msg[] = 'success';
            $msg[] = sprintf($GLOBALS['MESSAGES']['MSG_SEITE_GELOESCHT'], $_REQUEST['name']);
        } else {
            $msg[] = 'error';
            $msg[] = $GLOBALS['MESSAGES']['MSG_SEITE_NICHT_GELOESCHT'];
        }
        return $msg;
    }
        ...
        foreach ($_POST as $key => $value) {
            switch ($key) {
                case 'name':
                    if ($value == '') {
                        $this->mMsg[] = 'error';
                        $this->mMsg[] = $GLOBALS['MESSAGES']['MSG_SEITENTITEL_EINGEBEN'];
                    }
                    break;
                case 'datum_von':
                ...
                case 'kurztext':
                    if ($value == '') {
                        $this->mMsg[] = 'error';
                        $this->mMsg[] = $GLOBALS['MESSAGES']['MSG_KURZTEXT_EINGEBEN'];
                    }
                    break;
                case 'langtext':
                    if ($value == '') {
                        $this->mMsg[] = 'error';
                        $this->mMsg[] = $GLOBALS['MESSAGES']['MSG_LANGTEXT_EINGEBEN'];
                    }
                    break;
            }
        }
    ...
    public function save_page($page_id, $action = '', $dbaction = '') {
        ...
        if ($action == '' || $dbaction == '') {
            $this->mMsg[] = 'error';
            $this->mMsg[] = 'Unerlaubter Zugriff auf die DB!!';
            return $this->mMsg;
            exit;
        }
        ...
        if (isset($_POST['navid']) && $_POST['navid'] > 0) {
            ...
            $this->mMsg[] = 'error';
            $this->mMsg[] = 'Es fehlt die Navigations-Id - Speichern nicht m�glich!!';
            return $this->mMsg;
            exit;
        }
        ...
        if ($pagename == '') {
            $pagename = $GLOBALS['TEXTE']['TEXT_UNTERSEITE'] . $_POST['domain'];
        }
        ...
        switch (strtolower($dbaction)) {
            case 'insert':
                ...
                if (PEAR::isError($affected)) {
                    ...
                    $this->mMsg[] = 'error';
                    $this->mMsg[] = sprintf($GLOBALS['MESSAGES']['MSG_SEITE_NICHT_GESPEICHERT'], $_POST['name']);
                } else {
                    $this->mMsg[] = 'success';
                    $this->mMsg[] = sprintf($GLOBALS['MESSAGES']['MSG_SEITE_GESPEICHERT'], $_POST['name']);
...
//		return utf8_encode($data[$feldname]);
        ...
        if (count($unterseiten) > 0) {
            while ($row = $unterseiten->fetchRow(MDB2_FETCHMODE_ASSOC)) {
                ...
                if (stristr($inhalt1, '<img')) {
                    $inhalt1 = str_replace('/media/', $newmedia, $row['langtext']);
                }
                ...
                if (stristr($bild2, '/media/')) {
                    $bild2 = str_replace('/media/', $newmedia, $row['bild2']);
                }
                ...
                if ($inhalt1 != $row['langtext'] || $inhalt2 != $row['inhalt2'] || $bild1 != $row['bild1'] || $bild2 != $row['bild2']) {
                    ...
                    if (PEAR::isError($affected)) {
                        $msg[] = 'error';
                        $msg[] = sprintf($GLOBALS['MESSAGES']['MSG_SEITE_NICHT_GESPEICHERT'], $seite);
                    } else {
                        $msg[] = 'success';
                        $msg[] = sprintf($GLOBALS['MESSAGES']['MSG_SEITE_GESPEICHERT'], $seite);
                    }
                } else {
                    $msg[] = 'neutral';
                    $msg[] = 'OK: ' . $seite;
    ...
    private function check_datum($datum) {
        ...
        if (preg_match("^([0-9]{1,2})\.([0-9]{1,2})\.([0-9]{4})$", trim($datum))) {
            ...
            if (!checkdate($monat, $tag, $jahr)) {
                ...
                $this->mMsg[] = 'error';
                $this->mMsg[] = sprintf($GLOBALS['MESSAGES']['MSG_DATUM_UNGUELTIG'], $datum);
            }
        } else {
            $this->mMsg[] = 'error';
            $this->mMsg[] = $GLOBALS['MESSAGES']['MSG_DATUM_FORMAT'];
        }
    }
    ...
    private function compare_von_bis_datum($datum_von, $datum_bis) {
        ...
        if ($bis < $von) {
            $this->mMsg[] = 'error';
            $this->mMsg[] = sprintf($GLOBALS['MESSAGES']['MSG_DATUMBIS_KLEINER_DATUMVON'], $datum_bis, $datum_von);
        }
    }
