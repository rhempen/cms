...
switch ($action) {
    case 'LOGIN':
        ...
        if ((int) $rc == 0) {
            $message = sprintf($success, $GLOBALS['TEXTE']['WELCOME'] . $user);
            $action = 'MENU';
        }
        if ((int) $rc == 8) {
            $message = sprintf($error, $GLOBALS['TEXTE']['LOGIN_FAIL']);
        }
        if ((int) $rc == 10) {
            $message = sprintf($error, $GLOBALS['TEXTE']['NO_MENU'], $user);
        }
        break;
    ...
    case 'CHANGE':
        ...
        if ((int) $rc == 0) {
            $message = sprintf($success, $GLOBALS['TEXTE']['PASSWORD_CHANGED']);
            $action = 'LOGIN';
        }
        if ((int) $rc == 8) {
            $message = sprintf($error, $GLOBALS['TEXTE']['ALLE_FELDER_FUELLEN']);
        }
        if ((int) $rc == 9) {
            $message = sprintf($error, $GLOBALS['TEXTE']['PASSWORD_NEW_EQALS_OLD']);
        }
        if ((int) $rc == 10) {
            $message = sprintf($error, $GLOBALS['TEXTE']['PASSWORD_LENGTH']);
        }
        if ((int) $rc == 11) {
            $message = sprintf($error, $GLOBALS['TEXTE']['PASSWORD_EQUAL']);
        }
        if ((int) $rc == 12) {
            $message = sprintf($error, $GLOBALS['TEXTE']['PASSWORD_CHANGE_FAILED']);
        }
        if ((int) $rc == 14) {
    ...
    case 'FORGOT':
        ...
        if ($_SERVER['HTTP_HOST'] == 'hero') {
            $zusatz = ' New: ' . $_SESSION['newpass'];
        }
        if ((int) $rc == 0) {
            $message = sprintf($success, $GLOBALS['TEXTE']['EMAIL_SENT'] . $email);
            $action = 'LOGIN';
        }
        if ((int) $rc == 8) {
            $message = sprintf($error, $GLOBALS['TEXTE']['ALLE_FELDER_FUELLEN']);
        ...
        if ((int) $rc == 9) {
            $message = sprintf($error, $GLOBALS['TEXTE']['WRONG_EMAIL'] . $email);
        }
        if ((int) $rc == 10) {
            $message = sprintf($error, $GLOBALS['TEXTE']['EMAIL_SEND_FAILED'] . $email . $zusatz);
        }
        if ((int) $rc == 12) {
            $message = sprintf($error, $GLOBALS['TEXTE']['PASSWORD_CHANGE_FAILED'] . $zusatz);
        }
        if ((int) $rc == 14) {
            $msg = sprintf($GLOBALS['TEXTE']['USER_NOT_EXIST'], $user);
            $message = sprintf($error, $msg);
...
if ($menu != '')
    $menu_arr = $usmaint->set_menu_arr($menu);
