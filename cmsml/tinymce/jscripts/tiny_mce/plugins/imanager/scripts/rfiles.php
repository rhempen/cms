<?php
	...
	if ($param[0] == 'delete') {			// action: delete image
		...
		$action = $param[0];				
		$cfile  = $param[1];				// current filename	
	} else if ($param[0] == 'tbox') {			// action: update image list and select current image			
		$action = $param[0];				
		$cfile  = $param[1];	// current filename	
	}		
?>
...
// ============================================================
	function init() {		
		...
		if (action == 'upload') {
			...
			imageChangeClick();
		} else if (formObj.action.value == 'create') { // parent needs to be refreshed if directory got created				
...
// ============================================================	
	function imageChangeClick(obj) {
		...
		self.parent.imageChange(action);	// call parent function imageChange()
