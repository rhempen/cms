...
class File_IMC_Build_vCard extends File_IMC_Build {
    ...
    */
    function validateParam($name, $text, $comp = null, $iter = null)
    {
        ...
        if (preg_match('/[^a-zA-Z0-9\-]/i', $text)) {
        ...
        } elseif (isset($this->value['VERSION'])
        		and $this->value['VERSION'][0][0][0] == '2.1') {
            ...
            default:
        		...
        		and $this->value['VERSION'][0][0][0] == '3.0') {
    ...
    */
    ...
    function fetch()
    {
        ...
        if (!isset($this->value['VERSION']) or !is_array($this->value['VERSION'])) {
            return $this->raiseError(
        ...
        if (isset($this->value['NOTE']) &&
        	is_array($this->value['NOTE'])) {
            $lines[] = $this->getNote();
        }
        
        // prodid (3.0 only)
        if (isset($this->value['PERIOD']) &&
        	is_array($this->value['PRODID']) &&
            $this->value['VERSION'][0][0][0] == '3.0') {
        ...
        if (isset($this->value['URL']) &&
        	is_array($this->value['URL'])) {
            $lines[] = $this->getURL();
        }
        
        // class (3.0 only)
        if (isset($this->value['CLASS']) &&
        	is_array($this->value['CLASS']) &&
            $this->value['VERSION'][0][0][0] == '3.0') {
            $lines[] = $this->getClass();
        }
