...
/***********************************************************************************************
 * Verschiedene Funktionen auf der Übersicht ausführen
 ***********************************************************************************************/
// falls $page_id gesetzt ist, wird hier gleich zentral der ganze Datensatz für die Seite gelesen
// über $GLOBALS kann dann jederzeit darauf zugegriffen werden.
	if ($page_id > 0) {
	...
	switch ($action) {
		case 'create':
			...
			break;
		case 'delete':
			// 1. Bilder auf der Disk löschen
			// 2. Bilder in der DB löschen
			// 3. Seite löschen
		...
		case 'edit':
			...
			$page        = $pages->read_page($page_id);
			$view        = 'editPage';
			// einlesen der verfügbaren Thumbnails zur aktuellen Seite, aber nur im Update-Modus
			if ($dbaction != 'insert') {
...
//			echo urldecode($html);
		...
		case 'nameSave':
			if ($page_id > 0) {
				...
				$feldname	= 'name'; // Feldname, dessen wert geändert werden soll
				$message = $pages->page_update($page_id, $feldname, $feldwert);
				echo $message;
				$view = 'nothingToDo';
			}
 ...
 * Das Nachladen der Übersicht erfolgt via Javascript Reload
 ***********************************************************************************************/
