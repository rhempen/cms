...
if ($msent != 'sent' && $msent != 'fail' && $msent != 'code') {
	...
	<label for="name">'.$GLOBALS['KONTAKTFORM']['NAME'].'</label><input name="name" type="text" id="name" /><br />
	<label for="adresse">'.$GLOBALS['KONTAKTFORM']['ADRESSE'].'</label><input name="adresse" type="text" id="adresse" /><br />
	<label for="plz">'.$GLOBALS['KONTAKTFORM']['PLZ_ORT'].'</label><input name="plz_ort"type="text" id="plz_ort" /><br />
	<label for="telefon">'.$GLOBALS['KONTAKTFORM']['TELEFON'].'</label><input name="telefon" type="text" id="telefon" /><br />
	<label for="email">'.$GLOBALS['KONTAKTFORM']['EMAIL'].'</label><input name="email" type="text" id="email" /><br />
	<label for="kommentar">'.$GLOBALS['KONTAKTFORM']['MITTEILUNG'].'</label><textarea name="kommentar" cols="40" rows="4" id="kommentar"></textarea><br />
	<label for="bildcode">'.$GLOBALS['KONTAKTFORM']['BILDCODE'].'</label><input name="code" type="text" id="code" /><br />
	<label for="bild">&nbsp;</label><div class="inputWrapper"><img src="'.HOST.'frontend/captcha/bild.php' . '" /></div><br />
	<label for="schicken">&nbsp;</label><div class="inputWrapper"><input name="schicken" type="submit" class="submit" value="'.$GLOBALS['KONTAKTFORM']['SENDEN'].'" />
	<input name="resetten" type="reset" class="zuruecksetzen" value="'.$GLOBALS['KONTAKTFORM']['RESET'].'" /></div>
	</fieldset>';
} elseif ($msent == 'code') {
	$html .= '
	<form action="'.$nav_url.'" method="post" name="formmail" onsubmit="return check_form();">
