...
//		echo '$_SERVER[\'HTTP_REFERER\']: '. $_SERVER['HTTP_REFERER'];
    ...
    public function create_titel($kurztitel, $tplname = '', $tplnr = 0) {
        ...
        if ($rc) {
            $tpl->setCurrentBlock('seitentitel');
        }
    ...
    public function display_texte($row, $type = '') {
        ...
        if ($type == '') {
            ...
            if ($row['zusatztext'] != '') {
                $tpl->setVariable('zusatztext', '<p>' . $row['zusatztext'] . '</p>');
            }
            if ($row['kurztext'] != '') {
                $tpl->setVariable('kurztext', '<p>' . $row['kurztext'] . '</p>');
            }
        } else {
            if ($row['langtext'] != '') {
...
//		$max_hoehe	= $type == 'N' ? THUMB_NAVI_MAX_HEIGHT : THUMB_PAGE_MAX_HEIGHT;
        ...
        if (is_file($image)) {
            ...
            if ($imagesize == false || $imagesize == null)
                return $this->convert_rel2abs_path($bild);
        } else {
            return false;
        }
        ...
        if ($img_breite <= $th_breite && $img_hoehe <= $th_hoehe)
            return $this->convert_rel2abs_path($bild);
...
//			$tpl->parseCurrentBlock();
    ...
    public function erzeuge_bilderleiste($id, $type, $bild, $thumbsize) {
        ...
        while ($row = $bilder->fetchRow(MDB2_FETCHMODE_ASSOC)) {
            ...
            if ($thumb) {
                $thumb= $this->convert_rel2abs_path($thumb);
                $kommentar = ($row['kommentar'] != '' || $row['kommentar'] != null) ? $row['kommentar'] : '';
